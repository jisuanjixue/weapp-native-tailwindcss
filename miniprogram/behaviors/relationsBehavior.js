...
export default Behavior({
        ...
        created() {
            ...
            this.callDebounceFn = function (fn, wait = 0, options) {
                return (this._debounced =
                    this._debounced ||
                    debounce(fn.bind(this), wait, options)).call(this);
            };
    ...
    definitionFilter(defFields) {
        ...
        if (!isEmpty(relations)) {
            for (const key in relations) {
                ...
                methods.forEach((method) =>
                    bindFunc(relation, method, relation.observer),
                );
        ...
        Object.assign((defFields.methods = defFields.methods || {}), {
            getRelationsName: function (
                types = ['parent', 'child', 'ancestor', 'descendant'],
            ) {
                return Object.keys(relations || {})
                    .map((key) => {
                        if (
                            relations[key] &&
                            types.includes(relations[key].type)
                        ) {
                            return key;
                        }
                        return null;
