    ...
    return typeof miniprogramThis.getWindowInfo === 'function'
        ? keys.reduce(
              (acc, key) => ({
                  ...acc,
                  ...miniprogramThis[
                      `get${key.charAt(0).toUpperCase() + key.substring(1)}Info`
                  ](),
              }),
              {},
          )
...
export function getMenuButtonBoundingClientRectSync() {
    let menuRect;
    try {
        menuRect = miniprogramThis.getMenuButtonBoundingClientRect
            ? miniprogramThis.getMenuButtonBoundingClientRect()
            : null;
        if (menuRect === null) {
