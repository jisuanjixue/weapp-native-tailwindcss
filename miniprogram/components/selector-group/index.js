import * as common from '../../behaviors/wxs/common';
import baseComponent from '../../behaviors/baseComponent';
import classNames from '../../behaviors/classNames';
import eventsMixin from '../../behaviors/eventsMixin';
import fieldNamesBehavior from '../../behaviors/fieldNamesBehavior';
...
function getCheckedValues(newVal, oldVal = []) {
    ...
    checkedValues =
        checkedValues.indexOf(newVal) !== -1
            ? checkedValues.filter((n) => n !== newVal)
            : [...checkedValues, newVal];
    return checkedValues;
}

const getOptions = (options, fieldNames) =>
    common.getOptions(options, fieldNames);
...
baseComponent({
    ...
    computed: {
        classes: [
            'prefixCls, theme, shape',
            function (prefixCls, theme, shape) {
                const finalShape = ['rounded', 'rectangular'].includes(shape)
                    ? shape
                    : '';
    ...
    methods: {
        ...
        getValue(value = this.data.inputValue, _cols = this.data.options) {
            ...
            const checkedValues = value.reduce(
                (acc, val) => [
                    ...acc,
            ...
            const displayValue = checkedValues.map(
                (option) => option[labelName],
            );
