...
import runes2 from '../../behaviors/runes';
...
baseComponent({
    ...
    observers: {
        ['prefixCls, content, direction, rows, expandText, collapseText'](
            ...args
        ) {
            const [
                prefixCls,
    ...
    computed: {
        classes: [
            ...
            function (prefixCls) {
                ...
                const container = classNames(prefixCls, [
                    `${prefixCls}--container`,
                ]);
    ...
    methods: {
        ...
        check(left, right, props) {
            const { chars } = props;
            const end = props.content.length;
            const actionText = props.expanded
                ? props.collapseText
                : props.expandText;
            if (right - left <= 1) {
            ...
            return this.setDataPromise({ innerText })
                .then(() =>
                    Promise.all([this.getRootRef(), this.getContainerRef()]),
                )
                .then(([root, container]) => {
                    if (container.offsetHeight <= root.maxHeight) {
                        if (props.direction === 'end') {
        ...
        checkMiddle(leftPart, rightPart, props) {
            const { chars } = props;
            const end = props.content.length;
            const actionText = props.expanded
                ? props.collapseText
            ...
            const rightPartMiddle = Math.ceil(
                (rightPart[0] + rightPart[1]) / 2,
            );
            const innerText =
        ...
        getContainerRef() {
            return new Promise((resolve) => {
                ...
                query.select(`.${prefixCls}--container`).fields({
                    size: true,
